#!/bin/bash
  ...
  if [ -n "$ORACLE_23AI_HOME" ]
then # Find and loop through any dbconfig directories that don't belong to the current (default) database:
      for SID in "$(find "${DATA}"/dbconfig/ -maxdepth 1 -mindepth 1 | grep -v -E "\/${ORACLE_SID}$" | awk -F"/" '{print $NF}')"
       do # Add the oratab entry for this database if it doesn't exist:
          ...
          # Get the proper base/home directories:
            if [ -f "$ORACLE_HOME"/bin/orabaseconfig ]
          then export ORACLE_BASE_CONFIG=$("$ORACLE_HOME"/bin/orabaseconfig)/dbs
               export ORACLE_BASE_HOME=$("$ORACLE_HOME"/bin/orabasehome)
          else export ORACLE_BASE_CONFIG=$ORACLE_HOME/dbs
